...
class scpi:
    ...
    def sour_set(
        ...
        try:
            ...
            raise ValueError(
                f'{func.upper()} is not a defined waveform'
            ) from waveform_err
        ...
        try:
            ...
            raise ValueError(
                f'{trig.upper()} is not a defined trigger source'
            ) from trig_err
    ...
    def acq_set(
        ...
        if sample_format is not None:
            try:
                ...
                raise ValueError(
                    f'{sample_format.upper()} is not a defined format'
                ) from format_err
        ...
        try:
            ...
            raise ValueError(
                'Please select only one board option.',
                "'siglab' and 'input4' cannot be true at the same time.",
            ) from board_err
    ...
    def acq_data(
        ...
        if input4:
            try:
                ...
                raise ValueError(
                    'Channel needs to be either 1, 2, 3 or 4'
                ) from chanel_err
        else:
            try:
                assert chan in (1, 2)
    ...
    def uart_set(
        ...
        try:
            ...
            raise ValueError(
                f'Timeout {timeout} is out of range [0, 255]'
            ) from timeout_err
