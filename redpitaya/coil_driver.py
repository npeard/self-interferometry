...
class CoilDriver:
    ...
    def sample(self, waveform, randomize_phase_only=False, random_single_tone=False, equalize_gain=False, test_mode=False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Generate a sample waveform using the Waveform generator, with optional gain equalization.
        
        When equalize_gain is True, the spectrum is pre-compensated by dividing by the amplitude transfer function,
        ...
        Args:
            ...
            equalize_gain: If True, pre-compensate the spectrum to equalize the gain across frequencies
        ...
        """
        # If equalize_gain is False, simply return the output of Waveform.sample()
        if not equalize_gain:
            return waveform.sample(randomize_phase_only=randomize_phase_only, random_single_tone=random_single_tone)
        ...
        # Pre-compensate the spectrum by simply dividing by the complex transfer function
        equalized_spectrum = np.where(
            transfer_function != 0,
            voltage_spectrum / transfer_function,
            0
        )
        
        # Convert to time domain
        equalized_voltage = np.real(ifft(equalized_spectrum, norm="ortho"))
        equalized_voltage = np.fft.fftshift(equalized_voltage)
        
        # Recompute the spectrum and phase for use in other modules
        # First, shift back to match the original order
        y_unshifted = np.fft.ifftshift(equalized_voltage)
        
        # Compute the FFT to get the normalized spectrum
        normalized_spectrum = fft(y_unshifted, norm="ortho")
        
        return t, equalized_voltage, normalized_spectrum
