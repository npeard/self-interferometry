...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        losses = np.mean(
            (preds - truths) ** 2, axis=1
        )  # avg_mse_loss per buffer, [num_batches * batch_size]
        ...
        def plot_quartile_examples(quartiles, num_exs=4):
            ...
            for i in range(num_exs):
                for j, quartile in enumerate(quartiles):
                    ...
                    axes[i * 2 + 1, j].plot(
                        time_data[start_idxs],
                        quartile['preds'][i],
                        label=f'Pred (Avg MSE: {quartile["losses"][i]:.2f})',
                        color='orange',
                        marker='o',
                        markersize=2,
                    )
            ...
            fig.text(
                0.25, 0.98, '1st Quartile (Best Performance)', ha='center', fontsize=15
            )
            fig.text(
                0.75, 0.98, '4th Quartile (Worst Performance)', ha='center', fontsize=15
            )
